...
public class ChangeService {
  ...
  public List<Method> fetchChangeSets(final Class<?> type) throws LiquibaseParseException {
  ...
  private List<Method> filterChangeSetAnnotation(List<Method> allMethods) throws LiquibaseParseException {
    ...
    for (final Method method : allMethods) {
      if (method.isAnnotationPresent(ChangeSet.class)) {
        ...
        if (changeSetIds.contains(id)) {
          throw new LiquibaseParseException(String.format("Duplicated changeset id found: '%s'", id));
        }
        changeSetIds.add(id);
